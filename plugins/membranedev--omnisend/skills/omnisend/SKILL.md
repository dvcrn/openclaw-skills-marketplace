---
name: omnisend
description: "Omnisend integration. Manage Persons, Organizations, Activities, Notes, Files, Deals and more. Use when the user wants to interact with Omnisend data."
...
1. **Create a new connection:**
   ```bash
   membrane search omnisend --elementType=connector --json
   ```
...
1. **Check existing connections:**
   ...
   ```
   If a Omnisend connection exists, note its `connectionId`
...
When the available actions don't cover your use case, you can send requests directly to the Omnisend API through Membrane's proxy. Membrane automatically appends the base URL to the path you provide and injects the correct authentication headers — including transparent credential refresh if they expire.

...
- **Package-Based Trigger**: Trigger when user mentions `@trends-fun/trends-cli`, "trends command", or "trends tool".
- **Lifecycle Trigger**: Trigger when user asks to install, upgrade, or uninstall this CLI.
- **Error Trigger**: Trigger when user shares error messages containing "trends-cli" or related bonding curve failures.
...
- Trade flow
- Troubleshooting
2. Pick the matching recipe and provide runnable commands.
3. State expected observable outcomes.
4. Provide one concrete fallback path tied to known CLI behavior.
5. Recommend the next action only after the current step is verifiable.
...
- `trends config list|get|set|reset`
...
- Run quote first (`trends quote buy` or `trends quote sell`)
...
- Run `trends reward status` first
...
- Continue only with address-level operations, such as `trends wallet address`
...
### Commands to run
Provide copy-paste command blocks only with supported `trends` flags and subcommands.
...
- Create-specific constraints:
  - Do not suggest `--slippage-bps` for `trends create`.
...
- Reward-specific constraints:
  - `trends reward status` is read-only and should be the first check before claim.
  - `trends reward claim` should be suggested only when reward account exists and reward is greater than `0`.
...
- Include concrete verification commands (`trends --version`, `trends --help`, targeted `quote`/query checks) when relevant.
...
- If no exact match exists, run a minimal diagnosis set in this order:
  1. `trends --version`
  2. `trends --help`
  3. `trends config list`
  4. `trends wallet address`

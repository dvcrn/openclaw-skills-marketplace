...
export NOVITA_DOMAIN="sandbox.novita.ai"      # Optional, default shown
```

### 3. Verify

```bash
python3 -c "from novita_sandbox.code_interpreter import Sandbox; print('OK')"
```
...
```bash
python3 skills/novita-sandbox/scripts/sandbox.py create --template <template> --timeout <seconds>
```
...
```bash
python3 skills/novita-sandbox/scripts/sandbox.py exec <sandbox_id> "<command>" --timeout 60
```
...
```bash
# Read
python3 skills/novita-sandbox/scripts/sandbox.py read <sandbox_id> /home/user/output.txt

# Write (short content)
python3 skills/novita-sandbox/scripts/sandbox.py write <sandbox_id> /home/user/script.py "print('hello')"

# Write (multi-line via stdin)
cat <<'EOF' | python3 skills/novita-sandbox/scripts/sandbox.py write <sandbox_id> /home/user/run.sh --stdin
#!/bin/bash
...
```bash
# Local → Sandbox
python3 skills/novita-sandbox/scripts/sandbox.py upload <sandbox_id> ./local.txt /home/user/file.txt

# Sandbox → Local
python3 skills/novita-sandbox/scripts/sandbox.py download <sandbox_id> /home/user/result.png ./result.png
```

### Status / List / Kill

```bash
python3 skills/novita-sandbox/scripts/sandbox.py status <sandbox_id>
python3 skills/novita-sandbox/scripts/sandbox.py list
python3 skills/novita-sandbox/scripts/sandbox.py kill <sandbox_id>
```
...
cat <<'PYEOF' | python3 skills/novita-sandbox/scripts/sandbox.py write <sandbox_id> /home/user/browse.py --stdin
...
```bash
cat <<'PYEOF' | python3 skills/novita-sandbox/scripts/sandbox.py write <sandbox_id> /home/user/interact.js --stdin
...
python3 skills/novita-sandbox/scripts/sandbox.py create --template code-interpreter-v1 --timeout 600

# 2. Clone and build
python3 skills/novita-sandbox/scripts/sandbox.py exec abc123 \
  "git clone https://github.com/user/repo /home/user/project" --timeout 120
python3 skills/novita-sandbox/scripts/sandbox.py exec abc123 \
  "cd /home/user/project && npm install && npm test" --timeout 180

# 3. Read results
python3 skills/novita-sandbox/scripts/sandbox.py read abc123 /home/user/project/test-results.txt

# 4. Download artifacts if needed
python3 skills/novita-sandbox/scripts/sandbox.py download abc123 \
  /home/user/project/dist/output.zip ./output.zip

# 5. Let auto-pause handle cleanup, or kill
python3 skills/novita-sandbox/scripts/sandbox.py kill abc123
```
...
python3 skills/novita-sandbox/scripts/sandbox.py exec abc123 "echo hello"
```

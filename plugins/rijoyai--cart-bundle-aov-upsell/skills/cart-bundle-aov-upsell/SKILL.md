---
name: cart-bundle-aov-upsell
description: "Real-time bundle and cross-sell plans from the current cart to lift AOV, checkout upsells, and logical product pairing. Use this skill whenever the merchant wants higher AOV, new SKU launch sell-through, checkout bounce reduction with compelling add-ons, \"what to bundle with what's in cart,\" complementary recommendations, or one-click upsell copy at checkout. Metaphor: act as the store's shopping-guide brain — stitch isolated SKUs into coherent combos. Also trigger on cross-sell rules, complete-the-look, frequently bought together, or post-add-to-cart suggestions — even if they only say \"make people spend more per order.\" Do NOT use for unrelated SEO-only briefs, pure inventory sync, or pricing law/compliance drafting without a bundling ask."
---
...
2. **Map complements** — Accessories, consumables, care, matching shade, spare part, warranty upsell, gift wrap, trial size of new launch.
3. **Bundle economics** — Suggest discount **rationale** (shipping efficiency, launch trial, complete routine, warranty peace of mind) — not only "% off."
4. **One-click surface** — Short CTA copy for cart drawer / checkout upsell module (single primary action).
...
2. Margin guardrails: max discount % or floor price if known.
3. New launches to push (if any).
4. Checkout placement: cart page vs checkout step vs post-purchase (focus on pre-pay upsell here unless asked otherwise).
...
2. Combo table — mandatory columns (repeat rows).
3. Priority order — which upsell to show first (highest margin × fit).
4. A/B ideas — two rationales or two CTAs for the same pair.
5. What not to bundle — cannibalization or margin bleed.

...
2. **Repeat today**: 30/60/90-day repeat rate (or estimate)? Typical repurchase cycle?
3. **Existing loyalty / incentives**: Any points, tiers, or member benefits already? What’s working or broken?
4. **Channels**: Where do you reach customers (email, SMS, in-app, site, social)? What can trigger rewards (purchase, sign-up, review, referral)?
5. **Tech**: Loyalty app (e.g. Shopify Loyalty, Smile, Yotpo) or manual (coupons, segments)? Any limits?
6. **This round’s goal**: More repeat rate, higher LTV, better redemption, launch/relaunch program, or fix abuse/ROI?
7. **Margin & rules**: Willing to give what % off or equivalent in rewards? Any “do not do” (e.g. no stacking, no first-order discount)?
8. **Segments**: Any known segments (new vs repeat, high vs low value, lapsed) to tailor incentives?
...
- For loyalty model options and tier design patterns, see [references/loyalty_framework.md](references/loyalty_framework.md).
- For incentive types and calendar templates, see [references/incentive_playbook.md](references/incentive_playbook.md).
- For metric definitions and dashboard fields, see [references/metrics.md](references/metrics.md).
...
- Script: `scripts/loyalty_config_sanity_check.py`
...
- Example input: `scripts/loyalty_config.example.json`
...
- Script: `scripts/incentive_calendar_generator.py`
...
- Example input: `scripts/incentive_calendar_config.example.json`

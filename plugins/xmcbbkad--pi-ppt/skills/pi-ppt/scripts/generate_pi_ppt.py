...
def create_document(content: str, cards: int = 8, language: str = "cn") -> Dict[str, Any]:
    ...
    payload_with_sign = generate_signature_payload(PIPPT_APP_ID, PIPPT_APP_SECRET, timestamp, **payload)
...
if __name__ == "__main__":
    result = generate_pi_ppt(content="做一个关于比特币介绍的PPT", cards=8, language="zh")
    print(result)
    url = result.get("url")
    if not url:

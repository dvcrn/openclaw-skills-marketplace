...
- The conversion SDK must be installed first. Note: when using `pip install -i`, the index URL and package name must be specified separately. Available commands:
  ...
  # Windows / macOS
  python -m pip install --index-url https://test.pypi.org/simple/ ComPDFKitConversion
  ```
...
1. Confirm the Python package is installed:
   ...
   ```
2. Confirm `scripts/license.xml` is in place; the script will use the `scripts/` directory directly as the SDK `resource` path.
...
4. For more control, append common parameters:
   ...
   ```
5. On startup, the script first checks `scripts/license.xml`, reads the `<key>` field for SDK authentication, and uses the `scripts/` directory as the `resource` path.
...
- [ ] The script extracts the license key from the `<key>` field in `scripts/license.xml` for authentication
...
- [ ] When `license.xml` is missing or authentication fails, a clear error is output rather than a silent failure

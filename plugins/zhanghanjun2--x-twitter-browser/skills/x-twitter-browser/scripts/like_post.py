...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Like or Unlike an X tweet with Playwright browser auth")
    parser.add_argument("--tweet", required=True, help="Tweet URL (e.g. https://x.com/user/status/123) or tweet ID")
    parser.add_argument("--undo", action="store_true", help="Unlike (remove like) instead of like")
...
def open_tweet_and_like(page: Any, tweet_url: str, tweet_id: str, undo: bool, timeout_ms: int) -> None:
    ...
    if undo:
        btn = page.locator('[data-testid="unlike"], [aria-label="Unlike"]').first
    else:
        btn = page.locator('[data-testid="like"], [aria-label="Like"]').first
...
def main() -> None:
    ...
    with sync_playwright() as playwright:
        ...
        try:
            ...
            open_tweet_and_like(page, tweet_url, tweet_id, args.undo, args.timeout_ms)
            page.wait_for_timeout(2000)
            action = "Unlike" if args.undo else "Like"
            print(f"{action} flow executed. Check the tweet to confirm.")
        finally:
            context.close()
            browser.close()
